...
- **`orchestrator.md` is a special case: NOT a dispatchable type** — it governs the MAIN session's model choice (opus default, fable elevation criteria) and is read via the root CLAUDE.md §Orchestration pointer, never by the Agent-dispatch path (2026-08-10 P8).
