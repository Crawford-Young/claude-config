...
|---|---|---|
| Recon, existence checks, greps, single-fact read-and-report, doc fetches | **haiku** | n/a — unsupported |
...
| Verbatim transcription where the brief carries complete byte-for-byte code | **haiku** (orchestrator diff-verifies) | n/a — unsupported |
...
- Haiku's routing constraints — a dispatch that needs any of these is a mis-route, not a preference: 200K context ceiling (the only current model not at 1M); reliable knowledge cutoff Feb 2025 (training-data cutoff is a separate, later Jul 2025 — an unqualified "cutoff" is ambiguous); retirement not sooner than October 15, 2026 (currently Active, no deprecation notice).
