...
- **A toolchain major's Node floor gets checked against CI's `node-version` pin AND `engines` at bump time** — the local machine runs newer Node, so every local gate passes while CI dies at setup (pnpm 11.20 needs Node ≥22.13; CI pinned 20 → `ERR_UNKNOWN_BUILTIN_MODULE node:sqlite` at setup-node's store probe, first red only on the PR — 2026-08-09 creator-coach).
- **A minors sweep can install an npm-DEPRECATED broken minor** — `pnpm update` ignores deprecation (jest-dom 6.10.0: Node/peer breaking changes shipped as a minor, npm-deprecated). A caret range re-grabs it after manual downgrade — fix is an EXACT pin + a carried major-bump issue-log entry (2026-08-09 creator-coach H9; n=1, provisional).
- **Playwright version bumps pin new browser builds** — first e2e after the bump fails "Executable doesn't exist … chromium_headless_shell-<rev>"; `pnpm exec playwright install chromium` is part of the bump, not a debugging round (2026-08-09 creator-coach H9).
- **Tailwind v3→v4 in shadcn var-indirection repos: `npx @tailwindcss/upgrade` emits `hsl(var(--x))` inside plain `@theme` — dark mode silently breaks** (the var resolves once at `:root`; `.dark` overrides never propagate). Required pattern: `:root`/`.dark` blocks holding full `hsl(...)` values OUTSIDE `@layer base`, plus `@theme inline` mapping with plain `var()`. Pair tailwindcss.com's upgrade guide with ui.shadcn.com/docs/tailwind-v4 in the doc-recon brief — the shadcn page carries the token recipe the tailwind guide lacks — and visual-gate BOTH themes before commit (2026-08-09 creator-coach H7; every remaining v3 repo here faces this migration).

**Security:**
- Hardened security headers in `next.config.ts`
- `pnpm audit` in CI — no high/critical vulnerabilities
- **Transitive vuln override:** pin EXACT (never a range) the latest patched version INSIDE the consumer's declared range — latest-absolute can exceed it (fast-uri latest 4.1.2 vs ajv's `^3`; correct pin was 3.1.5 — 2026-08-09 creator-coach housekeeping). pnpm 11: overrides live in `pnpm-workspace.yaml` `overrides`, not package.json `pnpm.overrides`. Why-comment GHSA IDs + dependency chain + drop condition at the pin. `pnpm install` → `pnpm audit` to confirm. A red audit gate may be a same-day advisory, not the PR — check the advisory publish date before hunting the diff (2026-06-12).
