...
**A correct node drawn BEHIND an opaque one passes every assertion the suite can express — and the playtest gate cannot see it either.** Sibling order is the entire mechanism: a later sibling paints over an earlier one, so one integer decides whether a HUD exists on screen. Any wave adding a HUD element over a scene that paints an opaque background asserts DRAW ORDER (`hud.get_index() > host.get_index()`), not just visibility. The playtest half is why this needs a test at all: **a missing HUD element reads to a player as "not built yet," so it comes back as a feature request, not a bug report** — the one gate designed to catch what automation can't is structurally blind to absence. (dog-eat-dog wave 3a QA-R2e round 3, 2026-07-29: `ClockLabel` sat behind the drive's full-height opaque `Road` for the whole wave. Text, colour zones, spend arithmetic and the visibility flag were all asserted and all passed; two playtest rides missed it, and the user's eventual report was "show the time live on the driving game.")

**An invariant that holds only by arithmetic luck between two independently-tuned numbers is not an invariant — clamp it structurally.** Two tunables whose product must stay under a bound drift apart the moment either is retuned for feel, and the tuning session has no reason to check the other. Write the bound into the code as a clamp; do not pick numbers that happen to multiply safely. (dog-eat-dog wave 3a QA-R2e round 2, 2026-07-29: traffic could outrun the dog at 50.18 against 50.0 — band max `0.75` × per-car jitter draw `1.35` = `1.0125` of top speed. The generator had a speed FLOOR and no ceiling. A car faster than the dog is not an obstacle at all; it drives away and can never pin. Fixed with `clampf(v, top*0.05, top*0.95)`.)

**A feel lever that appears in docs as a LITERAL drifts every time it moves, and the sweep belongs to the tuning ROUND, not to wave close.** Keep the literal — player-facing prose that refuses to name the deadline is worse docs — but re-grep every doc for the old value in the same action batch that ships the new one. (dog-eat-dog wave 3a QA-R2e, 2026-07-29: `work_start_minutes` moved 510 → 525 → 480; `README.md` and repo `CLAUDE.md` still said **8:30** two rounds later — wrong since the first move, silent throughout. Second doc-drift class in one wave.)

---

## Geometry & Docs Invariants

- **Any wave that changes floor geometry re-verifies the FULL marker/respawn set live against the FINAL state — including every erosion/degradation loop — not against the state at placement time.** Markers are placed early; holes, chasms and shrink are authored later, by different tasks, and no per-task review can see the pairing because no task owns both ends. A respawn point over a hole is a softlock, and it passes every gate. Raycast down from each marker in the running physics space at each loop; report the sample count every run so a drifting probe window can't quietly shrink coverage. (what-is-dark 3b T9.4: 24 markers × 5 offsets × 2 loops verified clean — but 58 erodable bodies and 5 full-width chasms had been opened underneath them across three later tasks, and nothing had checked it until the whole-branch review.)
- **Doc claims about CONTROLS and INPUT are live-testable in one probe — test them, never review them by reading.** And the line a design ruling touches is the line most likely to be false: editing it under the ruling feels like verifying it. (n=2, both shipped past implementer + orchestrator + per-task review, both caught only at whole-branch review: phase-3a's erosion "air-only" wording against ground-scaling code; phase-3b's README "hold Jump to ride" against a ride that takes no input at all — `can_wallrun()` gates on speed alone. The 3b line had been rewritten *by that wave* to encode ruling #4.)
...
**Known blind spot: the gate cannot see ABSENCE.** A player reads a missing HUD element as unbuilt, not broken, so it returns as a feature request rounds later or never — see the draw-order gotcha above, which cost a full wave. Anything whose failure mode is "isn't there" needs an assertion, because the human gate is not the backstop for it. (dog-eat-dog wave 3a, 2026-07-29, n=1.)
