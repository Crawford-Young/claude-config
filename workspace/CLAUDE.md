...
> **Canonical location:** this file, the domain files, and the workspace reference docs live in the `claude-config` repo under `workspace/`, symlinked/junctioned into `~/code`. Edit through either path — same file. Commit changes in the `claude-config` repo via the worktree lane (§Branch Strategy commit protocol — never on the main checkout).
...
> **Worktree-always (2026-08-10 P8):** the MAIN CHECKOUT of every code repo (web/games/apps) never leaves `main` — ALL branch work happens in a worktree (`~/code/.worktrees/<repo>-<slug>`), keeping the main checkout a stable surface for second sessions, on-main QA, and the port map. `superpowers:using-git-worktrees` is the how; env-file copy per the `.worktreeinclude` rule below. Docs repo keeps its direct-commit lane; claude-config keeps its stricter protocol (below). Enforcement: prose this wave; branch-guard hook stays queued. **Migration deferral (2026-08-10):** four main checkouts sit off-main with in-flight branches — `web/scheduling-advisor@feat/chat-ux-w1`, `web/carsickyak-site@fix/audit-ignore-brace-expansion`, `games/dog-eat-dog@feat/wave-3b-evening-clock`, `games/what-is-dark@feat/phase4-wave1a`; each migrates (branch → worktree, checkout → main) at its next session touching that repo, not mid-flight.
...
- **New worktree: verify `.worktreeinclude` covers the repo's env files** (`.env`, `.env.local` — real secrets live in `.env.local`); repos without a `.worktreeinclude`, or worktrees created outside Claude Code's own tooling (plain `git worktree add`), still need the manual copy from the main checkout (2026-07-14; automated 2026-08-07 G52). Port collision is expected: `:3001` is fine for cookie-based QA; only fresh OAuth sign-in needs `:3000`. **Check the port holder (`netstat`) BEFORE launching any gate suite with e2e** — `reuseExistingServer: !process.env.CI` silently drives the other session's dev server (2026-07-16 eb3). A wave that ADDS an env var stales every pre-wave dev server (its process env lacks the var → runtime crashes masquerading as page regressions) — kill the holder before gates. Enumerate EVERY env-injection surface at the task adding the var: `.env.local`, `.env.example`, Vercel, playwright `webServer.env` dummies, and the CI workflow / e2e server-wrapper `env:` block — local e2e reads `.env.local`, so a missing dummy passes locally, fails in CI (staleness half 2026-07-26/27 oauth n=1 provisional; enumeration half n=2 FIRM since 2026-08-08 creator-coach W0 — `AUTH_SECRET` lived only in `.env`, e2e wrapper injected every var except it, local green, CI red with Auth.js MissingSecret even under SKIP_ENV_VALIDATION).
...
- **Two concurrent sessions on one repo MUST use separate worktrees** — includes the docs repo; a second session switching branches corrupts the first's in-flight gate (2026-06-12; 2026-07-01). claude-config: superseded by the commit protocol below — its main checkout never switches branches at all.
...
**claude-config commit protocol (2026-08-08): the main checkout never switches off `main` and never commits — it is the junction/live-routing surface only.** `skills/` and `workspace/` junctions load the MAIN checkout only, so live edits land on the main checkout's disk (Edit tool needs the real `claude-config/...` target path); verify `git -C <cfg> branch --show-current` = `main` first, abort if not (prose guard until the queued branch-guard hook + land script land — mechanization is its own cold-reviewed harness change). Every commit lands via an ephemeral worktree whose PATH contains `claude-config` (keeps the relocation-gate hook's path-match firing): `git -C <cfg> fetch origin && git -C <cfg> worktree add ~/code/.worktrees/claude-config-<wave-slug> -b chore/<wave-slug>-<desc> origin/main` (wave slug guards concurrent-session collisions). Re-apply YOUR change there from a path-scoped diff of your files only (`git -C <cfg> diff -- <your paths>`) — never copy whole files or the full diff out of the dirty main checkout; at N parallel sessions that sweeps other sessions' in-flight edits into your PR, the `git add -A` class. Baseline-CLAUDE.md rewrites also update `scripts/verify-relocation.mjs` `INTENTIONAL_EDITS` in the same payload. Commit → **user approval** → push, PR; any change made in the worktree during review is mirrored back to the main checkout in the same batch (the main checkout IS what routes). `git worktree remove` BEFORE `gh pr merge --rebase --delete-branch` — a branch checked out in a worktree can't be deleted (removal is safe: no node_modules; else `git-recovery-ops`). Sync the main checkout in ONE call: `git -C <cfg> fetch origin`, verify your live edit matches merged content (`git -C <cfg> diff origin/main --numstat -- <files>` empty; non-empty = reconcile, never discard), then `git -C <cfg> restore --source=HEAD --worktree -- <files> && git -C <cfg> merge --ff-only origin/main`. Never `git pull --rebase` here — it refuses on ANY unstaged change, and this checkout permanently carries other sessions' live edits; `merge --ff-only` proceeds around unrelated dirty files and stops only when incoming commits touch a locally-modified path, the correct stop. An abandoned/rejected PR = revert or re-land the live edit; never leave a routing edit with no commit behind it. Retires the foreign-branch-deferral class by convention (2026-07-02; 2026-07-28 two-payload day; PR #12 precedent).
...
Every domain defines its own gate list — see the domain's `CLAUDE.md`. Universal: nothing is "done" until domain gates pass at 100%, repo `README.md` + `CLAUDE.md` updated, `.gitignore` + `.env.example` current, no dead code, and `claude-md-management:reflect` has run at wave close **before** requesting push/PR. A reflect run loads the `harness-editing` skill BEFORE its MD-edit phase touches any agent-facing MD — junction targets, live-vs-commit lanes, and probe discipline bind reflect edits too (2026-08-10 P8; mirrored in the stop-reflect-gate hook message). Repo-level doc edits land in the wave branch, never a second PR.

Run `superpowers:verification-before-completion` before declaring anything done.

### 7. Context Hygiene

- Stop at `<!-- COMPACT POINT -->` markers — **marker stop is absolute**: blanket task approval never waives it; only an explicit instruction to skip does (2026-07-01). **Then choose, not default-/compact (2026-08-10 P8 audit):** state fully on disk (checklist ticked, issue log current, live edits enumerated) → hand continuation prompt + `/clear`; unrecorded conversational state remains → hand the focused `/compact` string. Pre-finalization markers default to `/clear`+continuation — the boundary `/clear` is minutes away and a compact there is double-billed (6/6 post-P3 manual compacts were marker-driven, 2 were exactly this collision — `docs/harness-evolution/research/2026-08-10-compact-clear-audit.md`).
...
- **Long-lived processes launch through a plain `run_in_background` call — never `nohup … & sleep N; echo started`** — the reported exit belongs to the echo AND the detached child dies unwatched. **Liveness-check before handing any URL over** (`curl -s -o /dev/null -w '%{http_code}' <url>`) — a dead server and a foreign server on the same port are indistinguishable from the URL (2026-07-29 AdSense W1; pairs with the port-ownership block above). A handoff URL must ALSO be the exact deep link to the action the user was asked to perform, driven once to first render yourself (Playwright, or curl following redirects — completing a third-party flow is NOT required, live-round clearance rules apply) — a live server plus a guessed route hands the user a 404 with your name on it (2026-08-08 creator-coach W0, user-corrected: sign-in QA handed `/api/auth/signin`, which 404'd; user burned the round hunting for the login. n=1, provisional).
...
Main-session model routing → `claude-config/agents/profiles/orchestrator.md` (opus default, fable elevation criteria — 2026-08-10 P8).
