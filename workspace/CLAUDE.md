...
Governs **every project in this workspace**. Carries the stack-independent rules: workflow order, planning discipline, git strategy, commit policy, context hygiene, orchestration, security baselines. **Stack rules live in the domain file** — Claude Code loads every `CLAUDE.md` from cwd upward, root first, each overriding the last on conflict.
...
- **Rebase-only workflow — linear history required:** `git fetch origin && git rebase origin/main` (never `git merge main`); `git pull --rebase` (never plain `pull`); `git config --global pull.rebase true`. **PR merge: "Rebase and merge" ONLY — never squash** (user directive 2026-07-21). `gh pr merge <n> --rebase --delete-branch`.
  - **Historical-base case (bases squash-merged before 2026-07-21 only):** a branch on a squash-merged base with diverged main must be squash-INTEGRATED onto fresh main — `rebase --onto` replays already-merged work and conflicts on every touched file. Sequence: `git checkout -B <branch> origin/main` → `git checkout <wave-head> -- <wave files>` (excluding files main also changed) → `git merge-file` those few → REGENERATE colliding migrations via `drizzle-kit generate` (never hand-edit snapshot JSON) → `pnpm install` → full gate → one integration commit. History loss fine. (2026-07-17 chat wave B: 30 commits, clean via this route.)
...
- `/clear` prompted by `reflect` at project end only. PreCompact hook archives full transcripts to `~/.claude/compact-archives/` (2026-07-27 — safety net; marker discipline unchanged). `/rewind` partial summarize is optional, never replaces markers.
- Always `Grep` before `Read`; always pass `offset`+`limit` to `Read`. >10 files read → offload remaining research to a subagent. Never re-read files already summarized in the conversation.
