...
- **Undelivered-report recovery (G72, 2026-08-09).** An agent that idles WITHOUT delivering after completing its work, where a report-only nudge also fails: extract the final assistant text from `~/.claude/projects/<slug>/subagents/agent-<name>-*.jsonl` — the full report is usually intact there (17.6k-char review recovered zero-loss). Grade on the recovered artifact; log the delivery failure in the perf MD. Second occurrence of the idle-without-delivery class (first: profile-documented); transcript extraction is the documented rung after nudge, before any re-dispatch.
