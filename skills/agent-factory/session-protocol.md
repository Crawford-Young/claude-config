...
- Every `--bg` dispatch carries `--max-budget-usd <n>` + `--max-turns <n>` ceilings (v2.1.212+: budget covers subagent spend and halts running bg agents — structural backstop for the live-LLM-clearance rule) AND `--forward-subagent-text` — without it the judging orchestrator sees only tool calls, no child reasoning or deviations, and the redo protocol judges blind. (code:agent-sdk/agent-loop.md + code:headless.md, 2026-08-08 P5 G67/G68.)
